...
- Embedding/vector search (Phase 2 in original roadmap)
...
3. Set `tombstone = TRUE`, increment deleting agent's clock entry via `JSON_SET(vector_clock, '$.<agentName>', COALESCE(JSON_EXTRACT(vector_clock, '$.<agentName>'), 0) + 1)`. Do not hard-delete.
...
> **Why not a separate idempotency table?** The write_id is scoped to `(space_id, key_name)` — one active key, one active write_id at a time. A separate table would require a JOIN on every write. Storing on the row is simpler and consistent with the single-row-lock pattern.
...
- Validate on staging before applying to production. Rollback script: `ALTER TABLE memories DROP COLUMN vector_clock, DROP COLUMN origin_agent, DROP COLUMN tombstone, DROP COLUMN last_write_id, DROP COLUMN last_write_snapshot, DROP COLUMN last_write_status; DROP INDEX idx_tombstone, idx_last_write_id ON memories;`
...
Existing methods `List`, `GetByID`, `GetByKey` gain a `tombstone = FALSE` filter in their SQL.
...
func (s *MemoryService) Create(ctx context.Context, spaceID, agentName, content, key string, tags []string, clock map[string]uint64, writeID string) (*WriteResult, error)
```
- If `clock == nil`: LWW fast path — overwrite unconditionally, return `WriteResult{Memory: m, Dominated: false}`.
- If `clock != nil`: transactional `SELECT ... FOR UPDATE`, compare, `TieBreak` for concurrent case, set `tombstone = FALSE` if reviving, commit with retry.
...
`createMemoryRequest` gains optional `clock` and `write_id` fields:
...
`CreateMemoryInput` gains optional `clock` and `write_id` fields. `Memory` type gains `clock`, `origin_agent`, `tombstone` fields (additive — existing `version`, `source`, `updated_by` unchanged).

`MnemoClient.store()` behavior:
...
- Add `tombstone = FALSE` filter to all read queries
- Retry on deadlock (same 3-attempt policy as Phase C)
- No client changes needed — same 204 response
- Integration tests: delete→read=404, repeated-delete=204, delete→list excludes tombstoned, agentName clock increment verified in DB
- **Dependencies:** Phase A.

### Phase C: Vector Clock Merge (~220 LoC)
- Implement `MergeVectorClocks`, `CompareClocks`, `TieBreak` in service layer
- Update `Create`: LWW fast path (no clock) + transactional CRDT path (with clock)
- `SELECT ... FOR UPDATE`, clock compare, tombstone revival, `TieBreak` for concurrent case
...
- Update OpenClaw `Memory` type (additive fields: `clock`, `origin_agent`, `tombstone`)
- Update `CreateMemoryInput` with optional `clock` and `write_id`
- `MnemoClient.store()` reads flat `Memory` response — no unwrapping needed
